...
public class MouseHoverPC : MonoBehaviour
{
    ...
    private bool userLog = true;            // Variabile che permette di capire se il login è stato fatto
    ...
    {
        // Se il login non è stato fatto quando il puntatore del mouse punta l'oggetto, esso cambia colore
        if (userLog)
        {
    ...
    {
        // Se il login non è stato fatto quando il puntatore del mouse non punta l'oggetto torna ad avere il colore originale
        if (userLog)
        {
    ...
    {
        // Se il login non è stato fatto attiva l'interfaccia del login per permettere l'autenticazione
        if (userLog)
        {
            // Imposta le variabili a false per non far comparire il menù di pausa durante il login
